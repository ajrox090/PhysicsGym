...
class BurgersFixedSetEnvGym(BurgersEnvGym):
    def __init__(self, data_path, data_range, once=True, **burgers_env_kwargs):
    ...
    def _get_frames_from_dataset(self, dataset):
        frames = np.array(
            [list(source.get('burgers_velocity', range(self.step_count + 1))) for source in dataset.sources],
            dtype=np.float32).squeeze(2)

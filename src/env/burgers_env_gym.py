...
from src.util.burgers_util import GaussianForce, GaussianClash
...
class BurgersEnvGym(gym.Env):
    ...
    def render(self, mode: str = 'live') -> None:
        if not self.test_mode:
            ...
            if mode == 'live':
                ...
                self.gifviz = GifPlotter('GymBurger-%s' % self.exp_name)
            else:
    ...
    def show_state_cmap(self, title='all velocities flow'):
    ...
    @staticmethod
    def show_state(states_list: List[Tuple[Union[np.ndarray, phiflow.BurgersVelocity], str]],
                   fig=None) -> Optional[plt.Figure]:
        setReturn = False
        if fig is None:
            fig = plt.figure().gca()
        else:
            setReturn = True
            fig = fig
        cmap = plt.cm.get_cmap('hsv', 100)  # list 1D
        # print(states_list)
        for (instate, title) in states_list:
            if isinstance(instate, phiflow.BurgersVelocity):
                instate = instate.velocity.data.reshape(instate.velocity.data.size)
            assert isinstance(instate, np.ndarray)
            fig.plot(np.linspace(-1, 1, len(instate.flatten())), instate.flatten(),
                     lw=2, color=cmap(random.randint(0, 100)), label=title)

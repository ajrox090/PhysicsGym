...
from src.env.gym_env_wrapper import GymEnvWrapper, VecEnvWrapper


class MonitorEnv(VecEnvWrapper):
    '''
    A monitor wrapper for OpenAI Gym environments, it is used to know the episode reward, length, time and other data.
    ...
    def __init__(
        ...
        if filename is None:
            ...
            if not filename.endswith(MonitorEnv.EXT):
                if os.path.isdir(filename):
                    filename = os.path.join(filename, MonitorEnv.EXT)
                else:
                    filename = filename + '.' + MonitorEnv.EXT
        ...
        self.curr_ep_data = {fn: np.zeros(1, dtype=np.float32) for fn in info_keywords}
        self.total_steps = 0
    ...
    def reset(self):
        ...
        obs = self.env.reset()
        self.curr_ep_rewards = np.zeros(1, dtype=np.float32)
        self.curr_ep_lengths = np.zeros(1, dtype=np.float32)
        self.episode_rewards = []
        self.episode_lengths = []
        self.episode_times = []
        ...
        for key in self.curr_ep_data:
            self.curr_ep_data[key] = np.zeros(1, dtype=np.float32)
        self.step_idx_in_rollout = 0
        return obs
